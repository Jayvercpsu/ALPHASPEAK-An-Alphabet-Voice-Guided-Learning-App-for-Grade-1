...
class _AlphabetScreenState extends State<AlphabetScreen> {
  ...
  int _currentPlayId = 0; // Unique ID to track the latest played sound

  Future<void> _speakCurrentLetter() async {
    int playId = ++_currentPlayId; // Assign a unique ID to this play session
    ...
    try {
      // Stop any ongoing speech and sound immediately
      ...
      // Check if another letter was clicked; if so, stop early
      if (playId != _currentPlayId) return;
...
// Ensure previous sounds stop before switching letters
  Future<void> _onLetterSelected(int index) async {
      await flutterTts.stop();
    await audioPlayer.stop();
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                style: GoogleFonts.poppins(fontSize: 55, color: Colors.white),
              ),
            ],
          ),
        ),
